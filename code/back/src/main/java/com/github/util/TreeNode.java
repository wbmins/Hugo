...
public class TreeNode {
     ...
     * @return　树
     */
    public static List<DataNode> getTreeList(Integer topId, List<DataNode> entityList) {
        //存储顶层的数据
        List<DataNode> resultList = new ArrayList<>();
        Map<Object, DataNode> treeMap = new HashMap<>();
        DataNode itemTree;
        ...
        for (int i = 0; i < entityList.size() && !entityList.isEmpty(); i++) {
            itemTree = entityList.get(i);
            //在map集合中寻找父亲
            DataNode data = treeMap.get(itemTree.getParentId());
            //判断父亲有没有
            if (data != null) {
